...
| `block-spec-doc-in-main-checkout` | PreToolUse / Write\|Edit\|MultiEdit | **deny** (fail-OPEN) | Blocks writing a wave spec doc (plan/architecture/design) under `docs/product-specs/` into the shared main checkout (`AAL_MAIN_REPO`) instead of the wave worktree (`AAL_WORKTREE_ROOT`). No-ops unless `AAL_MAIN_REPO` is set; node-absent → no-op. | bash, node |
| `block-audit-workflow-while-board-open` | PreToolUse / Workflow | **deny** (fail-CLOSED) | Blocks an audit-shaped `Workflow` while the active `BACKLOG.md` still has actionable `[QUEUED]`/`[IN-DEV]`/`[REVIEW]` cards — a fresh full-site/categorized audit runs only on a cleared board. Intent judged on the Workflow's declared identity + `meta` literal (not the whole script). Board via `AAL_BACKLOG` / `CLAUDE_PROJECT_DIR`; node-absent → deny. | bash, node |

...
big_file()    { awk 'BEGIN{c="";for(i=0;i<1000;i++)c=c"x";for(j=0;j<260;j++)printf "%s",c}' > "$1"; }
...
small_file "$PA/.claude/autoloop-log-2026-07-09.md"
big_file   "$PA/.claude/autoloop-log-2026-07-10-archive-01.md"
...
DN=$(mktemp -d); build_hookdir "$DN"; PN="$DN/proj"; mkdir -p "$PN/.claude"
...
SID8="a1b2c3d4"; SIDJSON="a1b2c3d4efgh"
...
big_file "$PF/.claude/autoloop-log-2026-07-10-99999999.md"
...
big_file "$PE/.claude/autoloop-log-2026-07-10-$SID8.md"
big_file "$PE/.claude/autoloop-log-2026-07-10.md"
E0=$(count_logs "$PE"); run_hook_sid "$DE" "$PE" "shortid"; E1=$(count_logs "$PE")

...
RULER="$(cd "$HOOKS_SRC/.." && pwd)/bin/sanitize-check.sh"
assert_no_cjk() {
  if printf '%s' "$2" | node -e '
const fs = require("node:fs");
const m = fs.readFileSync(process.argv[1], "utf8").match(/^const CJK = (\/.*\/);$/m);
if (!m) { console.error("assert_no_cjk: the CJK predicate is not readable from the ruler"); process.exit(2); }
const RE = eval(m[1]);
let s = "";
process.stdin.on("data", (d) => s += d).on("end", () => process.exit(RE.test(s) ? 1 : 0));
' "$RULER"; then
    ok "$1"
  else
    bad "$1 — a CJK codepoint reached the operator | got: [$2]"
  fi
}
...
assert_no_cjk       "AC3 pre-dispatch deny carries no CJK codepoint"        "$PDE"
assert_contains     "AC3 pre-review deny teaches the card skeleton (### [)"     "$PRE" "### ["
assert_no_cjk       "AC3 pre-review deny carries no CJK codepoint"          "$PRE"
rm -rf "$DE"

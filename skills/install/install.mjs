...
const target = resolve(expandTilde(args.target) || (process.env.CLAUDE_CONFIG_DIR || join(homedir(), ".claude")));
...
const TEMPLATE_FILES = [
  // 🔴 All FOUR rule files, because the managed CLAUDE.md block names all four. Copying two left an
  // adopter with a framework block pointing at `absence-and-tokens.md` and `gate-authoring.md` that
  // were never written — and nothing said so: the installer smoke test asserted the same two names
  // this list held, so the list and its check agreed with each other instead of with what ships.
  ...
  { src: "rules/common/absence-and-tokens.md", dst: "rules/common/absence-and-tokens.md", policy: "sidecar" },
  { src: "rules/common/gate-authoring.md", dst: "rules/common/gate-authoring.md", policy: "sidecar" },
...
let wrote = 0;
for (const s of staged) {
  if (s.status === "skip-exists") continue;
